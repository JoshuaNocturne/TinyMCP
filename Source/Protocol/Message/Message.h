...
public:
  Message(MessageType eMsgType, MessageCategory eCategory, bool bNeedIdentity)
    : eMessageType(eMsgType), eMessageCategory(eCategory) {
    if (bNeedIdentity) {
  ...
  bool bExist{ true };  // ¿ÉÑ¡Ê±¶ÔÏóÊÇ·ñ´æÔÚ
  unsigned long long ullTimestamp{ 0 };  // ºÁÃë¼¶Ê±¼ä´Á
  unsigned long ulRuntimeId{ 0 };  // ÔËÐÐÊ±id
