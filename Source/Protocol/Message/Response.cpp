...
int InitializeResult::DoSerialize(Json::Value& jMsg) const {
  ...
  auto fnSerializeMember = [&jResult](const auto& objMember,
                             const char* lpcszKey) -> int {
...
bool InitializeResult::IsValid() const {
  ...
  if (strProtocolVersion.empty() || !implServerInfo.IsValid() ||
...
bool CallToolResult::IsValid() const {
  if (vecTextContent.empty() && vecImageContent.empty() &&
      vecEmbeddedResource.empty())
    return false;
